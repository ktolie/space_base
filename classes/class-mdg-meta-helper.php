...
class MDG_Meta_Helper extends MDG_Meta_Form_Fields {
	...
	public function make_form( $args = array() ) {
		...
		foreach ( $meta_fields as $field ) {
			...
			echo $this->select_form_field( $field, $meta );
			echo '</td></tr>';
		} // foreach()
	...
	public function save_custom_meta( $post_id = null, $custom_meta_fields = array() ) {
		...
		$multi_inputs = $this->_merge_multi_input_fields();
		...
		foreach ( $custom_meta_fields as $field ) {
			...
			if ( $type == 'multi_input' ) {
				$new = isset( $multi_inputs[$id] ) ? $multi_inputs[$id] : '';
			} else {
	...
	/**
	 * Merging multi input fields.
	 *
	 * @return  array  The multi input fields.
	 */
	private function _merge_multi_input_fields() {
		$meta_fields    = $this->get_custom_meta_fields();
		$multi_fields   = array();
		$prefix         = $this->multi_input_prefix;
		$input_values   = ( isset( $_POST ) ) ? $_POST : array();
		$sorted_fields  = array();
		$meta_field_ids = array();
		$merge_fields   = array();
		$order          = array();

		// Order is driven by hidden inputs
		// So we have to grab those for use later
		// on after everything is add from the start position.
		foreach ( $_POST as $key => $value ) {
			if ( strpos( $key, "{$prefix}_order_" ) !== false ) {
				intval( $value );
				$order_key = str_replace( "{$prefix}_order_fieldcount", '', $key );
				$field_key = substr( $order_key, strpos( $order_key, '_' ) + 1 );
				$order_key = substr( $order_key, 0, strpos( $order_key, '_' ) );

				$order[$field_key]             = ( isset( $order[$field_key] ) ) ? $order[$field_key] : array();
				$order[$field_key][$order_key] = $value;
			} // if()
		}

		// Get the meta field ids
		foreach ( $meta_fields as $meta_field ) {
			if ( $meta_field['type'] != 'multi_input' ) {
				continue;
			} // if()

			$meta_field_ids[] = ( isset( $meta_field['id'] ) ) ? $meta_field['id'] : '';
		} // foreach()

		// Match the posted inputs to the meta ids
		foreach ( $meta_field_ids as $meta_field_id ) {
			foreach ( $input_values as $key => $value ) {
				if ( strpos( $key, "_{$meta_field_id}_" ) != false ) {
					$sorted_fields[$meta_field_id][$key] = $value;
				} // if()
			} // foreach()
		} // foreach()
		...
		foreach ( $sorted_fields as $sorted_field_key => $sorted_field ) {
			$merge_fields[$sorted_field_key] = array();
			foreach ( $sorted_field as $key => $value ) {
				$merge_key   = str_replace( "{$prefix}_", '', $key );
				$field_count = str_replace( 'fieldcount', '', substr( $merge_key, 0, strpos( $merge_key, '_' ) ) );
				$merge_key   = str_replace( "fieldcount{$field_count}_", '', $merge_key );
				$field_num   = substr( $merge_key, 0, strpos( $merge_key, '_' ) );
				$merge_key   = substr_replace( $merge_key, '', 0, strpos( $merge_key, '_' ) + 1 );
				$merge_key   = str_replace( "{$sorted_field_key}_", '', $merge_key );
				$merge_fields[$sorted_field_key][$field_count][$merge_key] = $value;
			} // foreach()
		} // foreach()

		// Put the merged fields in the new correct order
		foreach ( $merge_fields as $merge_field_key => $merge_field ) {
			$multi_fields[$merge_field_key] = array();
			foreach ( $merge_field as $key => $value ) {
				$order_key = $order[$merge_field_key][$key];
				$order_key = ( isset( $order_key ) ) ? $order_key : '';
				$multi_fields[$merge_field_key][$order_key] = $value;
			} // foreach()
		} // foreach()

		return $multi_fields;
	} // _merge_multi_input_fields()
